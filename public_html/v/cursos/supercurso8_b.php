...
// Busca status do curso na tabela `courses`
$slug = 'supercurso8'; // defina o slug deste curso conforme gravado em `courses.slug`
try {
    $stmt = $pdo->prepare("SELECT id, name, start_date, end_date, enrollment_open FROM courses WHERE slug = :slug LIMIT 1");
...
function isEnrollmentOpen($course) {
    ...
    $startOk = empty($course['start_date']) || $now <= new DateTime($course['start_date']);
    $endOk = empty($course['end_date']) || $now <= new DateTime($course['end_date']);
    $flag = (int)($course['enrollment_open'] ?? 0) === 1;
    return $flag && $endOk; // inscrições abertas até a data de fim, se houver
}

$enrollment_open = isEnrollmentOpen($course);
$cta_label = $enrollment_open ? 'Quero me inscrever agora!' : 'Inscrições encerradas';
$cta_href = $enrollment_open ? 'https://pay.hotmart.com/W101846156U?checkoutMode=6&off=jgybfc1g&offDiscount=SC8-LISTA' : '#';
$cta_class = 'hero-cta-btn' . ($enrollment_open ? '' : ' disabled');
?>
?>
...
<div class="main-content">
    ...
    <a href="<?= htmlspecialchars($cta_href) ?>" class="hero-cta-btn"><?= htmlspecialchars($cta_label) ?></a>
<?php else: ?>
    <span class="hero-cta-btn" style="background: #777; box-shadow:none; animation:none; cursor:not-allowed; opacity:.7;">Inscrições encerradas</span>
<?php endif; ?>
    </div>
    ...
    <a href="<?= htmlspecialchars($cta_href) ?>" class="hero-cta-btn"><?= htmlspecialchars($cta_label) ?></a>
<?php else: ?>
    <span class="hero-cta-btn" style="background: #777; box-shadow:none; animation:none; cursor:not-allowed; opacity:.7;">Inscri�f5es encerradas</span>
<?php endif; ?>
    </div>
    </div>
    </section>
...
<?php endif; ?>
    ...
    <p>3, 5, 11, 12, 17, 19, 24 e 26 de novembro e 1 e 3 de dezembro.</p>
    </div>
    <div class="detail-card">
    ...
    <a href="https://pay.hotmart.com/W101846156U?checkoutMode=6&off=jgybfc1g&offDiscount=SC8-LISTA">
    <img src="/images/cursos/supercurso8/lista.png" alt="Oferta de últimas vagas do Supercurso de Tradução de Jogos">
    </a>
    </div>
    </section>
